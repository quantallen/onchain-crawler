...
def pretty_table(dataframe):
    ...
    return table
bot = telegram.Bot(token=('5365972524:AAHzL0ZQLYzQNRRbLdXOi6HzM43X0agQ0OU'))
chat_id = '-857886086'
select_Name = {
                ...
                if project_name == 'OHM':
                    ...
                    for d in d_u :
                        if datetime.now() - d <= timedelta(minutes= 3):
                            ...
                            all_contents = get_latest_news(d,d_u[d])
                            score = trend_analysis(all_contents)
                            if d_f[d] not in exist_title :
                                df = [project_name,d_f[d],d_u[d],score]
                    ...
                    for u in u_t :
                        ...
                        if datetime.now() - d <= timedelta(minutes= 3):
                            ...
                            if u_t[u] not in exist_title :
                                df = [project_name,u_t[u],'https://sips.synthetix.io'+u,score]
                                telegram_send_meg(df)
                                #_store_in_db(df,id_ticker_data_dic)
